...
if __name__ == "__main__":
    ...
    ec = EllipticCurve(FieldElement(p, k, [1,0], irreducible_poly), FieldElement(p, k, [0,0], irreducible_poly))
    ...
    # A sends public keys
    B.getPublicKeys(A.sendPublicKeys())
    C.getPublicKeys(A.sendPublicKeys())
    # B sends public keys
    A.getPublicKeys(B.sendPublicKeys())
    C.getPublicKeys(B.sendPublicKeys())
    # C sends public keys
    A.getPublicKeys(C.sendPublicKeys())
    B.getPublicKeys(C.sendPublicKeys())

    print("I am A and I am going to print my keys")
    print(A.publicKeys)
